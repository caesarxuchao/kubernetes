...
diff -NauprBw "${DOCROOT}" "${TMP_DOCROOT}" > ${_tmp}/.diff && ret=0 || ret=$?
cat ${_tmp}/.diff | sed '/^-<!-- BEGIN MUNGE:.*/,/^-<!-- END MUNGE:.*/d' | sed '/###### Auto generated by spf13\/cobra/d' | sed '/^-\s*$/d'
rm -rf "${_tmp}"
...
then
  echo "${DOCROOT} up to date."
    ...
    echo "${DOCROOT} is out of date. Please run hack/run-gendocs.sh."
    echo  "If the diff shows kubectl commands split to multiple lines in kubectl's documentation, you need to manually split those commands in corresponding go files."
  exit 1
fi
