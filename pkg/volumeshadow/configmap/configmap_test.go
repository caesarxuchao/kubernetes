...
import (
	...
	"k8s.io/kubernetes/pkg/volumeshadow/empty_dir"
...
func newTestHost(t *testing.T, clientset clientset.Interface) (string, volumeshadow.VolumeHost) {
...
func TestCanSupport(t *testing.T) {
	pluginMgr := volumeshadow.VolumePluginMgr{}
	...
	if !plugin.CanSupport(&volumeshadow.Spec{Volume: &v1.Volume{VolumeSource: v1.VolumeSource{ConfigMap: &v1.ConfigMapVolumeSource{LocalObjectReference: v1.LocalObjectReference{Name: ""}}}}}) {
		t.Errorf("Expected true")
	}
	if plugin.CanSupport(&volumeshadow.Spec{}) {
...
func TestPlugin(t *testing.T) {
	...
	mounter, err := plugin.NewMounter(volumeshadow.NewSpecFromVolume(volumeSpec), pod, volumeshadow.VolumeOptions{})
	if err != nil {
	...
	vName, err := plugin.GetVolumeName(volumeshadow.NewSpecFromVolume(volumeSpec))
	if err != nil {
...
func doTestCleanAndTeardown(plugin volumeshadow.VolumePlugin, podUID types.UID, testVolumeName, volumePath string, t *testing.T) {
