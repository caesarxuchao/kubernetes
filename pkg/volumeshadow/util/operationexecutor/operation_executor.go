...
import (
	...
	"k8s.io/kubernetes/pkg/volumeshadow/util/nestedpendingoperations"
	volumetypes "k8s.io/kubernetes/pkg/volumeshadow/util/types"
	"k8s.io/kubernetes/pkg/volumeshadow/util/volumehelper"
)
...
type OperationExecutor interface {
	...
	// back off on retries.
	VerifyControllerAttachedVolume(volumeToMount VolumeToMount, nodeName k8stypes.NodeName, actualStateOfWorld ActualStateOfWorldAttacherUpdater) error
...
func NewOperationExecutor(
	...
	volumePluginMgr *volumeshadow.VolumePluginMgr,
...
type ActualStateOfWorldMounterUpdater interface {
	...
	MarkVolumeAsMounted(podName volumetypes.UniquePodName, podUID types.UID, volumeName v1.UniqueVolumeName, mounter volumeshadow.Mounter, outerVolumeSpecName string, volumeGidValue string) error
...
type ActualStateOfWorldAttacherUpdater interface {
	...
	MarkVolumeAsAttached(volumeName v1.UniqueVolumeName, volumeSpec *volumeshadow.Spec, nodeName k8stypes.NodeName, devicePath string) error

	// Marks the specified volume as detached from the specified node
	MarkVolumeAsDetached(volumeName v1.UniqueVolumeName, nodeName k8stypes.NodeName)

	// Marks desire to detach the specified volume (remove the volume from the node's
	// volumesToReportedAsAttached list)
	RemoveVolumeFromReportAsAttached(volumeName v1.UniqueVolumeName, nodeName k8stypes.NodeName) error

	// Unmarks the desire to detach for the specified volume (add the volume back to
	// the node's volumesToReportedAsAttached list)
	AddVolumeToReportAsAttached(volumeName v1.UniqueVolumeName, nodeName k8stypes.NodeName)
}
...
func (oe *operationExecutor) generateMountVolumeFunc(
	...
	var volumeAttacher volumeshadow.Attacher
