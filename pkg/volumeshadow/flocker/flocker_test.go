...
func newInitializedVolumePlugMgr(t *testing.T) (*volumeshadow.VolumePluginMgr, string) {
...
func TestPlugin(t *testing.T) {
	...
	mounter, err := plug.(*flockerPlugin).newMounterInternal(volumeshadow.NewSpecFromVolume(spec), types.UID("poduid"), fakeManager, fakeMounter)
	if err != nil {
...
func TestCanSupport(t *testing.T) {
	...
	specs := map[*volumeshadow.Spec]bool{
		&volumeshadow.Spec{
			Volume: &v1.Volume{
				VolumeSource: v1.VolumeSource{
					Flocker: &v1.FlockerVolumeSource{},
...
func TestNewMounterDatasetName(t *testing.T) {
	...
	_, err = plug.NewMounter(spec, &v1.Pod{}, volumeshadow.VolumeOptions{})
	assert.NoError(err)
}
...
func TestNewMounterDatasetUUID(t *testing.T) {
	...
	mounter, err := plug.NewMounter(spec, &v1.Pod{}, volumeshadow.VolumeOptions{})
	assert.NoError(err)
