...
var _ volumeshadow.Attacher = &azureDiskAttacher{}

var _ volumeshadow.AttachableVolumePlugin = &azureDataDiskPlugin{}
...
func (plugin *azureDataDiskPlugin) NewAttacher() (volumeshadow.Attacher, error) {
...
func (attacher *azureDiskAttacher) Attach(spec *volumeshadow.Spec, nodeName k8stypes.NodeName) (string, error) {
...
func (attacher *azureDiskAttacher) WaitForAttach(spec *volumeshadow.Spec, lunStr string, timeout time.Duration) (string, error) {
	volumeSource, err := getVolumeSource(spec)
	if err != nil {
		return "", err
	}
...
func (attacher *azureDiskAttacher) GetDeviceMountPath(spec *volumeshadow.Spec) (string, error) {
	volumeSource, err := getVolumeSource(spec)
	if err != nil {
		return "", err
	}
...
func (attacher *azureDiskAttacher) MountDevice(spec *volumeshadow.Spec, devicePath string, deviceMountPath string) error {
...
var _ volumeshadow.Detacher = &azureDiskDetacher{}

// NewDetacher initializes a volume Detacher
func (plugin *azureDataDiskPlugin) NewDetacher() (volumeshadow.Detacher, error) {
	azure, err := getAzureCloudProvider(plugin.host.GetCloudProvider())
	if err != nil {
		return nil, err
	}
...
func (detacher *azureDiskDetacher) Detach(diskName string, nodeName k8stypes.NodeName) error {
	if diskName == "" {
