...
func TestCanSupport(t *testing.T) {
	...
	if !plug.CanSupport(&volumeshadow.Spec{Volume: &v1.Volume{VolumeSource: v1.VolumeSource{AWSElasticBlockStore: &v1.AWSElasticBlockStoreVolumeSource{}}}}) {
		t.Errorf("Expected true")
	}
	if !plug.CanSupport(&volumeshadow.Spec{PersistentVolume: &v1.PersistentVolume{Spec: v1.PersistentVolumeSpec{PersistentVolumeSource: v1.PersistentVolumeSource{AWSElasticBlockStore: &v1.AWSElasticBlockStoreVolumeSource{}}}}}) {
		t.Errorf("Expected true")
	}
}
...
func TestPlugin(t *testing.T) {
	...
	mounter, err := plug.(*awsElasticBlockStorePlugin).newMounterInternal(volumeshadow.NewSpecFromVolume(spec), types.UID("poduid"), fakeManager, fakeMounter)
	if err != nil {
...
func TestMounterAndUnmounterTypeAssert(t *testing.T) {
	...
	mounter, err := plug.(*awsElasticBlockStorePlugin).newMounterInternal(volumeshadow.NewSpecFromVolume(spec), types.UID("poduid"), &fakePDManager{}, &mount.FakeMounter{})
	if _, ok := mounter.(volumeshadow.Unmounter); ok {
	...
	if _, ok := unmounter.(volumeshadow.Mounter); ok {
