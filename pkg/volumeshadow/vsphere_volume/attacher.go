...
var _ volumeshadow.Attacher = &vsphereVMDKAttacher{}
var _ volumeshadow.AttachableVolumePlugin = &vsphereVolumePlugin{}

// Singleton key mutex for keeping attach operations for the same host atomic
var attachdetachMutex = keymutex.NewKeyMutex()

func (plugin *vsphereVolumePlugin) NewAttacher() (volumeshadow.Attacher, error) {
...
// detach operations.
func (attacher *vsphereVMDKAttacher) Attach(spec *volumeshadow.Spec, nodeName k8stypes.NodeName) (string, error) {
...
func (attacher *vsphereVMDKAttacher) WaitForAttach(spec *volumeshadow.Spec, devicePath string, timeout time.Duration) (string, error) {
	volumeSource, _, err := getVolumeSource(spec)
	if err != nil {
		return "", err
	}
...
func (attacher *vsphereVMDKAttacher) GetDeviceMountPath(spec *volumeshadow.Spec) (string, error) {
	volumeSource, _, err := getVolumeSource(spec)
	if err != nil {
		return "", err
	}
...
func (attacher *vsphereVMDKAttacher) MountDevice(spec *volumeshadow.Spec, devicePath string, deviceMountPath string) error {
...
var _ volumeshadow.Detacher = &vsphereVMDKDetacher{}

func (plugin *vsphereVolumePlugin) NewDetacher() (volumeshadow.Detacher, error) {
	vsphereCloud, err := getCloudProvider(plugin.host.GetCloudProvider())
	if err != nil {
		return nil, err
	}
...
func (detacher *vsphereVMDKDetacher) Detach(deviceMountPath string, nodeName k8stypes.NodeName) error {
