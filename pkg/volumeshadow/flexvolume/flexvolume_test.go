...
func TestCanSupport(t *testing.T) {
	...
	if !plugin.CanSupport(&volumeshadow.Spec{Volume: &v1.Volume{VolumeSource: v1.VolumeSource{FlexVolume: &v1.FlexVolumeSource{Driver: "kubernetes.io/fakeAttacher"}}}}) {
		t.Errorf("Expected true")
	}
	if !plugin.CanSupport(&volumeshadow.Spec{PersistentVolume: &v1.PersistentVolume{Spec: v1.PersistentVolumeSpec{PersistentVolumeSource: v1.PersistentVolumeSource{FlexVolume: &v1.FlexVolumeSource{Driver: "kubernetes.io/fakeAttacher"}}}}}) {
		t.Errorf("Expected true")
	}
	if plugin.CanSupport(&volumeshadow.Spec{Volume: &v1.Volume{VolumeSource: v1.VolumeSource{}}}) {
...
func doTestPluginAttachDetach(t *testing.T, spec *volumeshadow.Spec, tmpDir string) {
	plugMgr := volumeshadow.VolumePluginMgr{}
	installPluginUnderTest(t, "kubernetes.io", "fakeAttacher", tmpDir, execScriptTempl1, nil)
	plugMgr.InitPlugins(ProbeVolumePlugins(tmpDir), volumetest.NewFakeVolumeHost(tmpDir, nil, nil, "" /* rootContext */))
	plugin, err := plugMgr.FindPluginByName("kubernetes.io/fakeAttacher")
...
func doTestPluginMountUnmount(t *testing.T, spec *volumeshadow.Spec, tmpDir string) {
	tmpDir, err := utiltesting.MkTmpdir("flexvolume_test")
	if err != nil {
		t.Fatalf("error creating temp dir: %v", err)
	}
...
func TestPluginVolumeAttacher(t *testing.T) {
	...
	doTestPluginAttachDetach(t, volumeshadow.NewSpecFromVolume(vol), tmpDir)
}
...
func TestPluginVolumeMounter(t *testing.T) {
	...
	doTestPluginMountUnmount(t, volumeshadow.NewSpecFromVolume(vol), tmpDir)
}
...
func TestPluginPersistentVolume(t *testing.T) {
	...
	doTestPluginAttachDetach(t, volumeshadow.NewSpecFromPersistentVolume(vol, false), tmpDir)
}
