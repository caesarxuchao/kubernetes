...
func (gceutil *GCEDiskUtil) CreateVolume(c *gcePersistentDiskProvisioner) (string, int, map[string]string, error) {
	...
	name := volumeshadow.GenerateVolumeName(c.options.ClusterName, c.options.PVName, 63) // GCE PD name can have up to 63 characters
	requestBytes := c.options.Capacity.Value()
	// GCE works with gigabytes, convert to GiB with rounding up
	requestGB := volumeshadow.RoundUpSize(requestBytes, 1024*1024*1024)
	...
	if zone == "" {
		...
		zone = volumeshadow.ChooseZoneForVolume(zones, c.options.PVCName)
	}
