...
func TestCanSupport(t *testing.T) {
	...
	if !plug.CanSupport(&volumeshadow.Spec{Volume: &v1.Volume{VolumeSource: v1.VolumeSource{Cinder: &v1.CinderVolumeSource{}}}}) {
		t.Errorf("Expected true")
	}

	if !plug.CanSupport(&volumeshadow.Spec{PersistentVolume: &v1.PersistentVolume{Spec: v1.PersistentVolumeSpec{PersistentVolumeSource: v1.PersistentVolumeSource{Cinder: &v1.CinderVolumeSource{}}}}}) {
		t.Errorf("Expected true")
	}
}
...
func getFakeDeviceName(host volumeshadow.VolumeHost, pdName string) string {
...
func TestPlugin(t *testing.T) {
	...
	mounter, err := plug.(*cinderPlugin).newMounterInternal(volumeshadow.NewSpecFromVolume(spec), types.UID("poduid"), &fakePDManager{0}, &mount.FakeMounter{})
	if err != nil {
