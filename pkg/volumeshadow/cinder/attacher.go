...
var _ volumeshadow.Attacher = &cinderDiskAttacher{}

var _ volumeshadow.AttachableVolumePlugin = &cinderPlugin{}

const (
	checkSleepDuration = time.Second
)

func (plugin *cinderPlugin) NewAttacher() (volumeshadow.Attacher, error) {
...
func (attacher *cinderDiskAttacher) Attach(spec *volumeshadow.Spec, nodeName k8stypes.NodeName) (string, error) {
...
func (attacher *cinderDiskAttacher) WaitForAttach(spec *volumeshadow.Spec, devicePath string, timeout time.Duration) (string, error) {
	volumeSource, _, err := getVolumeSource(spec)
	if err != nil {
		return "", err
	}
...
func (attacher *cinderDiskAttacher) MountDevice(spec *volumeshadow.Spec, devicePath string, deviceMountPath string) error {
...
var _ volumeshadow.Detacher = &cinderDiskDetacher{}

func (plugin *cinderPlugin) NewDetacher() (volumeshadow.Detacher, error) {
	cinder, err := getCloudProvider(plugin.host.GetCloudProvider())
	if err != nil {
		return nil, err
	}
...
func (detacher *cinderDiskDetacher) Detach(deviceMountPath string, nodeName k8stypes.NodeName) error {
