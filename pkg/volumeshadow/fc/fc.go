...
func ProbeVolumePlugins() []volumeshadow.VolumePlugin {
	return []volumeshadow.VolumePlugin{&fcPlugin{nil, exec.New()}}
}
...
var _ volumeshadow.VolumePlugin = &fcPlugin{}
var _ volumeshadow.PersistentVolumePlugin = &fcPlugin{}

const (
	fcPluginName = "kubernetes.io/fc"
)

func (plugin *fcPlugin) Init(host volumeshadow.VolumeHost) error {
	plugin.host = host
	return nil
}
...
func (plugin *fcPlugin) GetVolumeName(spec *volumeshadow.Spec) (string, error) {
...
func (plugin *fcPlugin) CanSupport(spec *volumeshadow.Spec) bool {
...
func (plugin *fcPlugin) NewMounter(spec *volumeshadow.Spec, pod *v1.Pod, _ volumeshadow.VolumeOptions) (volumeshadow.Mounter, error) {
	// Inject real implementations here, test through the internal function.
	return plugin.newMounterInternal(spec, pod.UID, &FCUtil{}, plugin.host.GetMounter())
}

func (plugin *fcPlugin) newMounterInternal(spec *volumeshadow.Spec, podUID types.UID, manager diskManager, mounter mount.Interface) (volumeshadow.Mounter, error) {
...
func (plugin *fcPlugin) NewUnmounter(volName string, podUID types.UID) (volumeshadow.Unmounter, error) {
	// Inject real implementations here, test through the internal function.
	return plugin.newUnmounterInternal(volName, podUID, &FCUtil{}, plugin.host.GetMounter())
}

func (plugin *fcPlugin) newUnmounterInternal(volName string, podUID types.UID, manager diskManager, mounter mount.Interface) (volumeshadow.Unmounter, error) {
...
func (plugin *fcPlugin) ConstructVolumeSpec(volumeName, mountPath string) (*volumeshadow.Spec, error) {
	fcVolume := &v1.Volume{
		Name: volumeName,
	...
	return volumeshadow.NewSpecFromVolume(fcVolume), nil
}
...
var _ volumeshadow.Mounter = &fcDiskMounter{}

func (b *fcDiskMounter) GetAttributes() volumeshadow.Attributes {
...
var _ volumeshadow.Unmounter = &fcDiskUnmounter{}
...
func getVolumeSource(spec *volumeshadow.Spec) (*v1.FCVolumeSource, bool, error) {
