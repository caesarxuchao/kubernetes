...
func (c *Client) ValidateCommon(t *testing.T, err error) {
	...
	for key, values := range c.Request.Query {
		...
		if !ok {
			switch key {
			case metav1.LabelSelectorQueryParam(scheme.Registry.GroupOrDie(api.GroupName).GroupVersion.String()):
				validator = ValidateLabels
			case metav1.FieldSelectorQueryParam(scheme.Registry.GroupOrDie(api.GroupName).GroupVersion.String()):
