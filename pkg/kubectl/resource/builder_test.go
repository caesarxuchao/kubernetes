...
{
    "kind": "Pod",
		"apiVersion": "` + scheme.Registry.GroupOrDie(api.GroupName).GroupVersion.String() + `",
    "metadata": {
        "name": "busybox{id}",
        "labels": {
            "name": "busybox{id}"
...
func TestPathBuilderAndVersionedObjectNotDefaulted(t *testing.T) {
	b := NewBuilder(testapi.Default.RESTMapper(), LegacyCategoryExpander, scheme.Scheme, fakeClient(), testapi.Default.Codec()).
...
func TestResourceByName(t *testing.T) {
	pods, _ := testData()
	b := NewBuilder(testapi.Default.RESTMapper(), LegacyCategoryExpander, scheme.Scheme, fakeClientWith("", t, map[string]string{
...
func TestResourceByNameWithoutRequireObject(t *testing.T) {
	b := NewBuilder(testapi.Default.RESTMapper(), LegacyCategoryExpander, scheme.Scheme, fakeClientWith("", t, map[string]string{}), testapi.Default.Codec()).
		NamespaceParam("test")
...
func TestSelector(t *testing.T) {
	pods, svc := testData()
	labelKey := metav1.LabelSelectorQueryParam(scheme.Registry.GroupOrDie(api.GroupName).GroupVersion.String())
	b := NewBuilder(testapi.Default.RESTMapper(), LegacyCategoryExpander, scheme.Scheme, fakeClientWith("", t, map[string]string{
...
func TestResourceTuple(t *testing.T) {
	...
	for k, testCase := range testCases {
		for _, requireObject := range []bool{true, false} {
			...
			b := NewBuilder(testapi.Default.RESTMapper(), LegacyCategoryExpander, scheme.Scheme, fakeClientWith(k, t, expectedRequests), testapi.Default.Codec()).
				NamespaceParam("test").DefaultNamespace().
...
func TestMultipleObject(t *testing.T) {
	...
	obj, err := NewBuilder(testapi.Default.RESTMapper(), LegacyCategoryExpander, scheme.Scheme, fakeClient(), testapi.Default.Codec()).
		NamespaceParam("test").Stream(r, "STDIN").Flatten().
		Do().Object()
...
func TestContinueOnErrorVisitor(t *testing.T) {
	...
	req := NewBuilder(testapi.Default.RESTMapper(), LegacyCategoryExpander, scheme.Scheme, fakeClient(), testapi.Default.Codec()).
		ContinueOnError().
		NamespaceParam("test").Stream(r, "STDIN").Flatten().
		Do()
	count := 0
...
func TestSingleItemImpliedRootScopedObject(t *testing.T) {
	...
	infos, err := NewBuilder(testapi.Default.RESTMapper(), LegacyCategoryExpander, scheme.Scheme, fakeClient(), testapi.Default.Codec()).
		NamespaceParam("test").DefaultNamespace().
		Stream(r, "STDIN").
		Flatten().
		Do().Infos()
...
func TestListObjectWithDifferentVersions(t *testing.T) {
	pods, svc := testData()
	labelKey := metav1.LabelSelectorQueryParam(scheme.Registry.GroupOrDie(api.GroupName).GroupVersion.String())
	obj, err := NewBuilder(testapi.Default.RESTMapper(), LegacyCategoryExpander, scheme.Scheme, fakeClientWith("", t, map[string]string{
		"/namespaces/test/pods?" + labelKey + "=a%3Db":     runtime.EncodeOrDie(testapi.Default.Codec(), pods),
		"/namespaces/test/services?" + labelKey + "=a%3Db": runtime.EncodeOrDie(testapi.Default.Codec(), svc),
	}), testapi.Default.Codec()).
		SelectorParam("a=b").
...
func TestWatch(t *testing.T) {
	_, svc := testData()
	w, err := NewBuilder(testapi.Default.RESTMapper(), LegacyCategoryExpander, scheme.Scheme, fakeClientWith("", t, map[string]string{
...
func TestWatchMultipleError(t *testing.T) {
	_, err := NewBuilder(testapi.Default.RESTMapper(), LegacyCategoryExpander, scheme.Scheme, fakeClient(), testapi.Default.Codec()).
		NamespaceParam("test").DefaultNamespace().
