...
func TestHelperList(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			Req: func(req *http.Request) bool {
				...
				if req.URL.Query().Get(metav1.LabelSelectorQueryParam(scheme.Registry.GroupOrDie(api.GroupName).GroupVersion.String())) != labels.SelectorFromSet(labels.Set{"foo": "baz"}).String() {
	...
	for _, test := range tests {
		...
		obj, err := modifier.List("bar", scheme.Registry.GroupOrDie(api.GroupName).GroupVersion.String(), labels.SelectorFromSet(labels.Set{"foo": "baz"}), false)
		if (err != nil) != test.Err {
			t.Errorf("unexpected error: %t %v", test.Err, err)
		}
		if err != nil {
