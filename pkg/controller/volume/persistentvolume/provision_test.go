	...
	tests := []controllerTest{
		{
			...
			novolumes,
			newVolumeArray("pvc-uid11-1", "1Gi", "uid11-1", "claim11-1", v1.VolumeBound, v1.PersistentVolumeReclaimDelete, annBoundByController, annDynamicallyProvisioned, annClass),
			newClaimArray("claim11-1", "uid11-1", "1Gi", "", v1.ClaimPending, annClass),
		...
		},
		{
			...
			novolumes,
			novolumes,
			newClaimArray("claim11-2", "uid11-2", "1Gi", "", v1.ClaimPending, annClass),
			newClaimArray("claim11-2", "uid11-2", "1Gi", "", v1.ClaimPending, annClass),
		...
		},
		{
			...
			novolumes,
			novolumes,
			newClaimArray("claim11-3", "uid11-3", "1Gi", "", v1.ClaimPending, annClass),
			newClaimArray("claim11-3", "uid11-3", "1Gi", "", v1.ClaimPending, annClass),
			[]string{"Warning ProvisioningFailed"}, noerrors,
		...
		},
		{
			...
			novolumes,
			novolumes,
			newClaimArray("claim11-4", "uid11-4", "1Gi", "", v1.ClaimPending, annClass),
			newClaimArray("claim11-4", "uid11-4", "1Gi", "", v1.ClaimPending, annClass),
			[]string{"Warning ProvisioningFailed"}, noerrors,
		...
		},
		{
			...
			newVolumeArray("volume11-6", "1Gi", "", "", v1.VolumePending, v1.PersistentVolumeReclaimRetain, annClass),
			newVolumeArray("volume11-6", "1Gi", "uid11-6", "claim11-6", v1.VolumeBound, v1.PersistentVolumeReclaimRetain, annBoundByController, annClass),
			newClaimArray("claim11-6", "uid11-6", "1Gi", "", v1.ClaimPending, annClass),
			newClaimArray("claim11-6", "uid11-6", "1Gi", "volume11-6", v1.ClaimBound, annClass, annBoundByController, annBindCompleted),
			noevents, noerrors,
		...
		},
		{
			...
			novolumes,
			newVolumeArray("pvc-uid11-7", "1Gi", "uid11-7", "claim11-7", v1.VolumeBound, v1.PersistentVolumeReclaimDelete, annBoundByController, annDynamicallyProvisioned, annClass),
			newClaimArray("claim11-7", "uid11-7", "1Gi", "", v1.ClaimPending, annClass),
			...
			wrapTestWithInjectedOperation(wrapTestWithProvisionCalls([]provisionCall{}, testSyncClaim), func(ctrl *PersistentVolumeController, reactor *volumeReactor) {
				...
				reactor.lock.Lock()
				volume := newVolume("pvc-uid11-7", "1Gi", "uid11-7", "claim11-7", v1.VolumeBound, v1.PersistentVolumeReclaimDelete, annBoundByController, annDynamicallyProvisioned, annClass)
		...
		},
		{
			...
			novolumes,
			newVolumeArray("pvc-uid11-8", "1Gi", "uid11-8", "claim11-8", v1.VolumeBound, v1.PersistentVolumeReclaimDelete, annBoundByController, annDynamicallyProvisioned, annClass),
			newClaimArray("claim11-8", "uid11-8", "1Gi", "", v1.ClaimPending, annClass),
			// Binding will be completed in the next syncClaim
			newClaimArray("claim11-8", "uid11-8", "1Gi", "", v1.ClaimPending, annClass),
			noevents,
			[]reactorError{
		...
		},
		{
			...
			novolumes,
			novolumes,
			newClaimArray("claim11-9", "uid11-9", "1Gi", "", v1.ClaimPending, annClass),
			newClaimArray("claim11-9", "uid11-9", "1Gi", "", v1.ClaimPending, annClass),
		...
		},
		{
			...
			novolumes,
			novolumes,
			newClaimArray("claim11-10", "uid11-10", "1Gi", "", v1.ClaimPending, annClass),
			newClaimArray("claim11-10", "uid11-10", "1Gi", "", v1.ClaimPending, annClass),
		...
		},
		{
			...
			novolumes,
			novolumes,
			newClaimArray("claim11-11", "uid11-11", "1Gi", "", v1.ClaimPending, annClass),
			newClaimArray("claim11-11", "uid11-11", "1Gi", "", v1.ClaimPending, annClass),
			[]string{"Warning ProvisioningFailed", "Warning ProvisioningCleanupFailed"},
			[]reactorError{
				// Inject error to five kubeclient.PersistentVolumes.Create()
		...
		},
		{
			...
			novolumes,
			novolumes,
			newClaimArray("claim11-12", "uid11-12", "1Gi", "", v1.ClaimPending, annClass),
			newClaimArray("claim11-12", "uid11-12", "1Gi", "", v1.ClaimPending, annClass),
			[]string{"Warning ProvisioningFailed"},
			[]reactorError{
				// Inject error to five kubeclient.PersistentVolumes.Create()
	...
	tests := []controllerTest{
		{
			...
			newClaimArray("claim14-1", "uid14-1", "1Gi", "", v1.ClaimPending, annAlphaClass),
			// Binding will be completed in the next syncClaim
			newClaimArray("claim14-1", "uid14-1", "1Gi", "", v1.ClaimPending, annAlphaClass),
		...
		},
		{
			...
			newClaimArray("claim14-2", "uid14-2", "1Gi", "", v1.ClaimPending, annAlphaClass),
			// Binding will be completed in the next syncClaim
			newClaimArray("claim14-2", "uid14-2", "1Gi", "", v1.ClaimPending, annAlphaClass),
			noevents, noerrors, wrapTestWithProvisionCalls([]provisionCall{provisionAlphaSuccess}, testSyncClaim),
		},
	...
	tests := []controllerTest{
		{
			...
			novolumes,
			newVolumeArray("pvc-uid12-1", "1Gi", "uid12-1", "claim12-1", v1.VolumeBound, v1.PersistentVolumeReclaimDelete, annBoundByController, annDynamicallyProvisioned, annClass),
			newClaimArray("claim12-1", "uid12-1", "1Gi", "", v1.ClaimPending, annClass),
			// Binding will be completed in the next syncClaim
			newClaimArray("claim12-1", "uid12-1", "1Gi", "pvc-uid12-1", v1.ClaimBound, annClass, annBoundByController, annBindCompleted),
			noevents, noerrors, wrapTestWithProvisionCalls([]provisionCall{provision1Success}, testSyncClaim),
		},
	}
