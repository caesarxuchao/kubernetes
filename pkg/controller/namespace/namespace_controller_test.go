...
func testSyncNamespaceThatIsTerminating(t *testing.T, versions *unversioned.APIVersions) {
	...
	for scenario, testInput := range scenarios {
		...
		err := syncNamespace(mockClient, clientPool, operationNotSupportedCache{}, groupVersionResources, testInput.testNamespace, v1.FinalizerKubernetes)
		if err != nil {
...
func TestRetryOnConflictError(t *testing.T) {
	...
	retryOnce := func(kubeClient clientset.Interface, namespace *v1.Namespace) (*v1.Namespace, error) {
		numTries++
		if numTries <= 1 {
			return namespace, errors.NewConflict(v1.Resource("namespaces"), namespace.Name, fmt.Errorf("ERROR!"))
		}
...
func TestSyncNamespaceThatIsActive(t *testing.T) {
	...
	err := syncNamespace(mockClient, nil, operationNotSupportedCache{}, testGroupVersionResources(), testNamespace, v1.FinalizerKubernetes)
	if err != nil {
