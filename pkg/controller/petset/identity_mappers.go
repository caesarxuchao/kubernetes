...
func (n *NetworkIdentityMapper) SetIdentity(id string, pet *v1.Pod) {
	pet.Annotations[podv1.PodHostnameAnnotation] = fmt.Sprintf("%v-%v", n.ps.Name, id)
	pet.Annotations[podv1.PodSubdomainAnnotation] = n.ps.Spec.ServiceName
	return
}
...
func (n *NetworkIdentityMapper) String(pet *v1.Pod) string {
	hostname := pet.Annotations[podv1.PodHostnameAnnotation]
	subdomain := pet.Annotations[podv1.PodSubdomainAnnotation]
...
// mappers.
func identityHash(ps *apps.PetSet, pet *v1.Pod) string {
	id := ""
