...
func newPetSetWithVolumes(replicas int, name string, petMounts []v1.VolumeMount, podMounts []v1.VolumeMount) *apps.PetSet {
...
func newPodList(ps *apps.PetSet, num int) []*v1.Pod {
...
func (f *fakePetClient) Delete(p *pcb) error {
	...
	for i, pet := range f.pets {
		if p.pod.Name == pet.pod.Name {
			found = true
			f.recorder.Eventf(pet.parent, v1.EventTypeNormal, "SuccessfulDelete", "pet: %v", pet.pod.Name)
			continue
		}
...
func (f *fakePetClient) Create(p *pcb) error {
	...
	f.recorder.Eventf(p.parent, v1.EventTypeNormal, "SuccessfulCreate", "pet: %v", p.pod.Name)
