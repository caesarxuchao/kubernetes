...
func TestNewGarbageCollector(t *testing.T) {
	...
	metaOnlyClientPool := dynamic.NewClientPool(config, scheme.Registry.RESTMapper(), dynamic.LegacyAPIPathResolverFunc)
	config.ContentConfig.NegotiatedSerializer = nil
	clientPool := dynamic.NewClientPool(config, scheme.Registry.RESTMapper(), dynamic.LegacyAPIPathResolverFunc)
	...
	gc, err := NewGarbageCollector(metaOnlyClientPool, clientPool, scheme.Registry.RESTMapper(), podResource)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGCListWatcher(t *testing.T) {
	...
	srv, clientConfig := testServerAndClientConfig(testHandler.ServeHTTP)
	defer srv.Close()
	clientPool := dynamic.NewClientPool(clientConfig, scheme.Registry.RESTMapper(), dynamic.LegacyAPIPathResolverFunc)
