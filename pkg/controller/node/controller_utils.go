...
func deletePods(kubeClient clientset.Interface, recorder record.EventRecorder, nodeName, nodeUID string, daemonStore cache.StoreToDaemonSetLister) (bool, error) {
	remaining := false
	selector := fields.OneTermEqualSelector(v1.PodHostField, nodeName)
...
func forcefullyDeleteNode(kubeClient clientset.Interface, nodeName string, forcefulDeletePodFunc func(*v1.Pod) error) error {
	selector := fields.OneTermEqualSelector(v1.PodHostField, nodeName)
	options := v1.ListOptions{FieldSelector: selector}
	pods, err := kubeClient.Core().Pods(v1.NamespaceAll).List(options)
	if err != nil {
...
func markAllPodsNotReady(kubeClient clientset.Interface, node *v1.Node) error {
	...
	opts := v1.ListOptions{FieldSelector: fields.OneTermEqualSelector(v1.PodHostField, nodeName)}
...
func terminatePods(kubeClient clientset.Interface, recorder record.EventRecorder, nodeName string, nodeUID string, since time.Time, maxGracePeriod time.Duration) (bool, time.Duration, error) {
	...
	for _, pod := range pods.Items {
		...
		if remaining < 0 {
			...
			recordNodeEvent(recorder, nodeName, nodeUID, v1.EventTypeNormal, "TerminatingEvictedPod", fmt.Sprintf("Pod %s has exceeded the grace period for deletion after being evicted from Node %q and is being force killed", pod.Name, nodeName))
			if err := kubeClient.Core().Pods(pod.Namespace).Delete(pod.Name, v1.NewDeleteOptions(0)); err != nil {
