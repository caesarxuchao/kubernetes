	...
	if nc.allocateNodeCIDRs {
		nodeEventHandlerFuncs = cache.ResourceEventHandlerFuncs{
			AddFunc: func(originalObj interface{}) {
				obj, err := v1.Scheme.DeepCopy(originalObj)
				if err != nil {
			...
			UpdateFunc: func(_, obj interface{}) {
				...
				if node.Spec.PodCIDR == "" {
					nodeCopy, err := v1.Scheme.Copy(node)
					if err != nil {
						utilruntime.HandleError(err)
						return
					}
		...
		for k := range nc.zoneTerminationEvictor {
			nc.zoneTerminationEvictor[k].Try(func(value TimedValue) (bool, time.Duration) {
				...
				if completed {
					...
					recordNodeEvent(nc.recorder, value.Value, nodeUid, v1.EventTypeNormal, "TerminatedAllPods", fmt.Sprintf("Terminated all Pods on Node %s.", value.Value))
					return true, 0
				}
