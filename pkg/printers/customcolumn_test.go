...
func TestNewColumnPrinterFromSpec(t *testing.T) {
	...
	for _, test := range tests {
		printer, err := printers.NewCustomColumnsPrinterFromSpec(test.spec, scheme.Codecs.UniversalDecoder(), test.noHeaders)
		if test.expectErr {
			if err == nil {
...
func TestNewColumnPrinterFromTemplate(t *testing.T) {
	...
	for _, test := range tests {
		...
		printer, err := printers.NewCustomColumnsPrinterFromTemplate(reader, scheme.Codecs.UniversalDecoder())
		if test.expectErr {
			if err == nil {
				t.Errorf("[%s] unexpected non-error", test.name)
			}
