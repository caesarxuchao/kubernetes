...
// 1. Declare your own registry.APIGroupRegistrationManager in your own test code.
...
func install() {
	...
	federationinstall.Install(groupFactoryRegistry, registry, scheme)
	appsinstall.Install(groupFactoryRegistry, registry, scheme)
	authenticationinstall.Install(groupFactoryRegistry, registry, scheme)
	authorizationinstall.Install(groupFactoryRegistry, registry, scheme)
	autoscalinginstall.Install(groupFactoryRegistry, registry, scheme)
	batchinstall.Install(groupFactoryRegistry, registry, scheme)
	certificatesinstall.Install(groupFactoryRegistry, registry, scheme)
	componentconfiginstall.Install(groupFactoryRegistry, registry, scheme)
	extensionsinstall.Install(groupFactoryRegistry, registry, scheme)
	imagepolicyinstall.Install(groupFactoryRegistry, registry, scheme)
	policyinstall.Install(groupFactoryRegistry, registry, scheme)
	rbacinstall.Install(groupFactoryRegistry, registry, scheme)
	settingsinstall.Install(groupFactoryRegistry, registry, scheme)
	storageinstall.Install(groupFactoryRegistry, registry, scheme)
}
...
func init() {
	...
	if len(kubeTestAPI) != 0 {
		...
		for i := len(testGroupVersions) - 1; i >= 0; i-- {
			...
			Groups[groupVersion.Group] = TestGroup{
				...
				internalTypes:        scheme.KnownTypes(internalGroupVersion),
				externalTypes:        scheme.KnownTypes(groupVersion),
			}
		}
	}

	if _, ok := Groups[api.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: api.GroupName, Version: registry.GroupOrDie(api.GroupName).GroupVersion.Version}
		Groups[api.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: api.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(api.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
	if _, ok := Groups[extensions.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: extensions.GroupName, Version: registry.GroupOrDie(extensions.GroupName).GroupVersion.Version}
		Groups[extensions.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: extensions.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(extensions.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
	if _, ok := Groups[autoscaling.GroupName]; !ok {
		internalTypes := make(map[string]reflect.Type)
		for k, t := range scheme.KnownTypes(extensions.SchemeGroupVersion) {
			if k == "Scale" {
				continue
			}
			internalTypes[k] = t
		}
		externalGroupVersion := schema.GroupVersion{Group: autoscaling.GroupName, Version: registry.GroupOrDie(autoscaling.GroupName).GroupVersion.Version}
	...
	if _, ok := Groups[batch.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: batch.GroupName, Version: registry.GroupOrDie(batch.GroupName).GroupVersion.Version}
		Groups[batch.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: batch.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(batch.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
	if _, ok := Groups[apps.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: apps.GroupName, Version: registry.GroupOrDie(apps.GroupName).GroupVersion.Version}
		Groups[apps.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: apps.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(apps.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
	if _, ok := Groups[policy.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: policy.GroupName, Version: registry.GroupOrDie(policy.GroupName).GroupVersion.Version}
		Groups[policy.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: policy.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(policy.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
	if _, ok := Groups[federation.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: federation.GroupName, Version: registry.GroupOrDie(federation.GroupName).GroupVersion.Version}
		Groups[federation.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: federation.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(federation.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
	if _, ok := Groups[rbac.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: rbac.GroupName, Version: registry.GroupOrDie(rbac.GroupName).GroupVersion.Version}
		Groups[rbac.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: rbac.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(rbac.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
	if _, ok := Groups[settings.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: settings.GroupName, Version: registry.GroupOrDie(settings.GroupName).GroupVersion.Version}
		Groups[settings.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: settings.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(settings.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
	if _, ok := Groups[storage.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: storage.GroupName, Version: registry.GroupOrDie(storage.GroupName).GroupVersion.Version}
		Groups[storage.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: storage.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(storage.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
	if _, ok := Groups[certificates.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: certificates.GroupName, Version: registry.GroupOrDie(certificates.GroupName).GroupVersion.Version}
		Groups[certificates.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: certificates.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(certificates.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
	if _, ok := Groups[imagepolicy.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: imagepolicy.GroupName, Version: registry.GroupOrDie(imagepolicy.GroupName).GroupVersion.Version}
		Groups[imagepolicy.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: imagepolicy.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(imagepolicy.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
	if _, ok := Groups[authorization.GroupName]; !ok {
		externalGroupVersion := schema.GroupVersion{Group: authorization.GroupName, Version: registry.GroupOrDie(authorization.GroupName).GroupVersion.Version}
		Groups[authorization.GroupName] = TestGroup{
			externalGroupVersion: externalGroupVersion,
			internalGroupVersion: authorization.SchemeGroupVersion,
			internalTypes:        scheme.KnownTypes(authorization.SchemeGroupVersion),
			externalTypes:        scheme.KnownTypes(externalGroupVersion),
		}
	}
...
func (g TestGroup) Codec() runtime.Codec {
	if serializer.Serializer == nil {
		return codecs.LegacyCodec(g.externalGroupVersion)
	}
	return codecs.CodecForVersions(serializer.Serializer, codecs.UniversalDeserializer(), schema.GroupVersions{g.externalGroupVersion}, nil)
}
...
func (g TestGroup) StorageCodec() runtime.Codec {
	...
	ds := recognizer.NewDecoder(s, codecs.UniversalDeserializer())

	return codecs.CodecForVersions(s, ds, schema.GroupVersions{g.externalGroupVersion}, nil)
}

// Converter returns the scheme for the API version to test against, as set by the
// KUBE_TEST_API env var.
func (g TestGroup) Converter() runtime.ObjectConvertor {
	interfaces, err := registry.GroupOrDie(g.externalGroupVersion.Group).InterfacesFor(g.externalGroupVersion)
	if err != nil {
		panic(err)
	}
...
func GetCodecForObject(obj runtime.Object) (runtime.Codec, error) {
	...
	for _, group := range Groups {
		...
		if scheme.Recognizes(kind) {
			return group.Codec(), nil
		}
	}
	// Codec used for unversioned types
	if scheme.Recognizes(kind) {
		serializer, ok := runtime.SerializerInfoForMediaType(codecs.SupportedMediaTypes(), runtime.ContentTypeJSON)
		if !ok {
