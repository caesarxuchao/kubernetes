...
func deepCloneService(svc *api.Service) *api.Service {
	value, err := scheme.Scheme.DeepCopy(svc)
	if err != nil {
...
func TestServiceStorageValidatesUpdate(t *testing.T) {
	...
	for _, failureCase := range failureCases {
		c, created, err := storage.Update(ctx, failureCase.Name, rest.DefaultUpdatedObjectInfo(&failureCase, scheme.Scheme))
		if c != nil || created {
...
func TestServiceRegistryUpdateExternalService(t *testing.T) {
	...
	if _, _, err := storage.Update(ctx, svc2.Name, rest.DefaultUpdatedObjectInfo(svc2, scheme.Scheme)); err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	...
	if _, _, err := storage.Update(ctx, svc3.Name, rest.DefaultUpdatedObjectInfo(svc3, scheme.Scheme)); err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
}
...
func TestServiceRegistryIPUpdate(t *testing.T) {
	...
	updated_svc, _, _ := storage.Update(ctx, update.Name, rest.DefaultUpdatedObjectInfo(update, scheme.Scheme))
	updated_service := updated_svc.(*api.Service)
	...
	_, _, err := storage.Update(ctx, update.Name, rest.DefaultUpdatedObjectInfo(update, scheme.Scheme))
...
func TestUpdateServiceWithConflictingNamespace(t *testing.T) {
	...
	ctx := genericapirequest.NewDefaultContext()
	obj, created, err := storage.Update(ctx, service.Name, rest.DefaultUpdatedObjectInfo(service, scheme.Scheme))
	if obj != nil || created {
