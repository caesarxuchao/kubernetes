...
func NewEtcdStorage(t *testing.T, group string) (*storagebackend.Config, *etcdtesting.EtcdTestServer) {
	server, config := etcdtesting.NewUnsecuredEtcd3TestClientServer(t, scheme.Scheme)
...
func New(t *testing.T, storage *genericregistry.Store) *Tester {
	return &Tester{
		tester:  resttest.New(t, storage, scheme.Scheme),
		storage: storage,
	}
}
...
func getCodec(obj runtime.Object) (runtime.Codec, error) {
	fqKinds, _, err := scheme.Scheme.ObjectKinds(obj)
	if err != nil {
	...
	if scheme.Scheme.Recognizes(scheme.Registry.GroupOrDie(api.GroupName).GroupVersion.WithKind(fqKind.Kind)) {
		codec = testapi.Default.Codec()
	} else if scheme.Scheme.Recognizes(testapi.Extensions.GroupVersion().WithKind(fqKind.Kind)) {
