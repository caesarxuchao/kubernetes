...
CONCURRENT_GC_SYNCS=${CONCURRENT_GC_SYNCS:-100}
...
CONTROLLER_MANAGER_TEST_ARGS="${CONTROLLER_MANAGER_TEST_LOG_LEVEL} ${TEST_CLUSTER_RESYNC_PERIOD} ${TEST_CLUSTER_API_CONTENT_TYPE} ${KUBEMARK_MASTER_COMPONENTS_QPS_LIMITS} --enable-garbage-collector=${ENABLE_GARBAGE_COLLECTOR} --concurrent-gc-syncs=${CONCURRENT_GC_SYNCS}"
