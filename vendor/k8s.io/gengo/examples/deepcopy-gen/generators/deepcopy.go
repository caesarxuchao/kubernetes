...
func Packages(context *generator.Context, arguments *args.GeneratorArgs) generator.Packages {
	...
	for i := range inputs {
		...
		// typesPkg is where the types are defined. Sometimes it is different
		// from pkg. For example, kubernetes core/v1 types are defined in
		// vendor/k8s.io/api/core/v1, while pkg is at pkg/api/v1.
		typesPkg := pkg
		...
		if ptag != nil {
			...
			// types are defined at different location from pkg
			if ptag.types != "" {
				typesLocation, err := context.AddDirAndReturnAddedPath(filepath.Join(pkg.Path, ptag.types))
				if err != nil {
					glog.Fatalf("cannot import package %s", ptag.types)
				}
				var ok bool
				...
				typesLocationToDCFnLocation[typesLocation] = pkg.Path
				...
				&generator.DefaultPackage{
					...
					GeneratorFunc: func(c *generator.Context) (generators []generator.Generator) {
						return []generator.Generator{
							NewGenDeepCopy(arguments.OutputFileBaseName, typesPkg.Path, pkg.Path, boundingDirs, (ptagValue == tagValuePackage), ptagRegister),
						}
					},
...
func NewGenDeepCopy(sanitizedName, typesPackage, outputPackage string, boundingDirs []string, allTypes, registerTypes bool) generator.Generator {
	return &genDeepCopy{
...
var typesLocationToDCFnLocation = make(map[string]string)
...
func (g *genDeepCopy) Namers(c *generator.Context) namer.NameSystems {
	...
	return namer.NameSystems{
		...
		"dcFnName": &dcFnNamer{
			public:     deepCopyNamer(),
			tracker:    g.imports,
			myPackage:  g.outputPackage,
			translator: typesLocationToDCFnLocation,
		},
	}
}
...
type dcFnNamer struct {
	...
	myPackage string
	// translates the package path of types to the package path of the
	// corresponding DeepCopy functions
	translator map[string]string
}

func (n *dcFnNamer) Name(t *types.Type) string {
	copyT := *t
	pubName := n.public.Name(&copyT)
	// translate the package path if the type is defined at a different location
	// from where the DeepCopy functions are defined
	if translated, ok := n.translator[copyT.Name.Package]; ok {
		copyT.Name.Package = translated
	}
	n.tracker.AddType(&copyT)
	if copyT.Name.Package == n.myPackage {
		return "DeepCopy_" + pubName
	}
	return fmt.Sprintf("%s.DeepCopy_%s", n.tracker.LocalNameOf(copyT.Name.Package), pubName)
}
...
func (g *genDeepCopy) Init(c *generator.Context, w io.Writer) error {
	...
	glog.V(5).Infof("Registering types in pkg %q", g.outputPackage)
